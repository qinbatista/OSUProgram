...
void dynarray_insert(struct dynarray* da, void* val)
{
   ...
   */
  da->data[(da->start + da->size) % da->capacity] = val;
  da->size++;
}
...
void* dynarray_get(struct dynarray* da, int idx)
{
  ...
  return da->data[(da->start + idx) % da->capacity];
}
...
void dynarray_set(struct dynarray* da, int idx, void* val)
{
  ...
  da->data[(da->start + idx) % da->capacity] = val;
}

int isEmpty(struct dynarray* da)
{
  if(da->size == 0)
  {
    return 1;
...
void addVal(struct dynarray* da, void* val)
{
  ...
  index = (da->size + da->start) % da->capacity;
...
void* topVal(struct dynarray* da)
{
...
void removeVal(struct dynarray* da)
{
  ...
  index = (da->start + 1) % da->capacity;

  if(da->size > 0 && da->size != 1)
  {
    da->start = index;
  }
